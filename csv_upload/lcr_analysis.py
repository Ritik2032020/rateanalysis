...
def load_csv_data(file_path):
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for row in reader:
            # Add a validation step for 'OperatorId'
            operator_id = row.get('OperatorId', '')
            if not operator_id.isdigit():
                continue  # Skip rows with non-integer 'OperatorId'
...
def find_lcr(data, country=None, operator=None):
    ...
    if country is not None:
        country_ids = {str(country)} if country is not None and country != '' else set()
        filtered_data = [row for row in filtered_data if row['CountryId'] in country_ids]

    if operator is not None and operator != '':
        operator_ids = {int(operator)}
    ...
    for row in filtered_data:
        ...
        rate = float(row['Rate'])
        vendor = {'VendorId': row['VendorId'], 'Rate': rate}
        
        if key not in lcr_results or rate < lcr_results[key]['rate']:
            lcr_results[key] = {'rate': rate, 'vendors': [vendor]}
        # elif rate == lcr_results[key]['rate']:
            # lcr_results[key]['vendors'].append(vendor)
        else:
            lcr_results[key]['vendors'].append(vendor)

    for key, result in lcr_results.items():
        result['vendors'] = sorted(result['vendors'], key=lambda x: x['Rate'])[:13]
